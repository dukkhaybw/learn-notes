...
重载函数是指一个函数同时拥有多个同名的函数签名。例如，一个函数拥有两个及以上的调用签名，或者一个构造函数拥有两个及以上的构造签名。当使用不同数量和类型的参数调用重载函数时，可以执行不同的函数实现代码。
...
**`infer`** 用于**在条件类型中推断（infer）未知类型**的关键字。作用是**在类型系统的模式匹配过程中，动态捕获并命名一个未知的子类型**，从而使类型系统能够基于现有类型结构生成新的类型。
...
当调用print的时候，传入"A"，编译器就会提示只能传"A"或"B"。这是因为IStr被明确地定义为这两个字符串字面量的联合类型，所以TypeScript知道只有这两个可能的有效值，所以在调用函数时，编辑器会给出这两个选项的提示。



```ts
type IStr = "A" | "B" | string;
...
将IStr的类型改成了"A" | "B" | string。这时候，当调用print("A")时，编译器就不再提示传入"A"或"B"了。为什么会这样？这里涉及到TypeScript的类型系统如何处理联合类型中的字面量类型和更广泛的类型，比如string。



TypeScript的类型拓宽（widening）和类型收缩的概念。

如果一个联合类型包含多个字面量类型和一个基础类型，如string，那么这些字面量类型会被认为是该基础类型的子类型，所以联合类型会被简化为该基础类型。例如，"A" | "B" | string实际上等于string类型（拓宽为string），因为所有可能的"A"和"B"都属于string类型。因此，当函数参数的类型是string时，调用函数时自然不会有具体字面量的提示，因为任何字符串都可以传入，而不仅仅是特定的几个。所以在这种情况下，联合类型中的字面量类型会被更广泛的类型覆盖，导致编译器不再保留字面量类型的提示信息。

如果联合类型中的某个成员不是基础类型的子类型，比如number和string的联合，这时候就不会被简化。

在 TypeScript 中，**联合类型中的更广泛类型会覆盖其子类型**，导致具体字面量的类型信息丢失。以下是问题的核心原因和机制：

第一种情况：`type IStr = "A" | "B"`

- **类型明确性**：`IStr` 被明确定义为仅允许 `"A"` 或 `"B"`。这两个是独立的字面量类型（literal types），编译器会保留它们的类型信息。
- **编译器提示**：当调用 `print("A")` 时，TypeScript 知道参数只能是这两个值，因此会提供精确的代码提示。

第二种情况：`type IStr = "A" | "B" | string`

- **类型简化**：`string` 是 `"A"` 和 `"B"` 的父类型。TypeScript 会将联合类型简化为 `string`，因为所有字面量都是 `string` 的子类型。等价于直接使用 `string`。
- **提示丢失**：参数类型被推断为 `string`，编译器不再保留具体的字面量信息，因此不会提示 `"A"` 或 `"B"`。

关键机制

- **类型覆盖**：当联合类型包含父类型（如 `string`）时，子类型（如 `"A"`）会被父类型“吸收”，导致联合类型退化为父类型。
- **类型提示依赖具体类型**：只有明确指定为字面量联合类型时，编译器才能提供具体的值提示。



**解决办法**

要解决这个问题并让 TypeScript 在调用 `print` 时提示 `"A"` 或 `"B"`，需要**避免让联合类型包含父类型（如 `string`）**。以下是具体修改方案：

------

情况 1：若**不需要其他字符串**，只需允许 `"A"` 或 `"B"`：

直接移除 `string` 类型即可：

```typescript
type IStr = "A" | "B"; // 仅允许 "A" 或 "B"
...
情况 2：若你**需要同时允许其他字符串**，但希望优先提示 `"A"` 或 `"B"`：

可以通过 **函数重载（Function Overload）** 实现：

```typescript
type IStr = "A" | "B" | string; // 允许任意字符串

// 函数重载声明
function print(str: "A" | "B"): void; // 优先提示具体值
function print(str: string): void;    // 通用类型
// 函数实现
function print(str: IStr) {
  console.log(str);
}

print("A"); // ✅ 优先提示 "A" | "B"，但允许传入任意字符串
```

此时调用 `print` 时：

- 如果输入字面量（如 `"A"`），编译器会优先提示 `"A" | "B"`。
- 如果输入其他字符串（如 `"C"`），则类型检查依然通过。
...
在 TypeScript 中，当使用 `string & {}` 时，实际上是通过 **类型交集（Intersection Type）** 创造了一个特殊的类型标记，这会阻止 TypeScript 的联合类型简化机制。

核心原理：**阻止联合类型简化**

1. **默认行为**：
   当联合类型包含 `string` 时，所有更具体的字面量类型（如 `"A"`）会被视为 `string` 的子类型，导致 TypeScript 将联合类型简化为 `string`：

   ```typescript
   type IStr = "A" | "B" | string; // 被简化为 `string`
   ```

2. **使用 `string & {}` 的魔法**：

   - `string & {}` 的交集类型会被 TypeScript 视为一个 **非原始字符串类型**（尽管逻辑上等价于 `string`）。

   - 这种写法会欺骗编译器，使其认为 `string & {}` 是一个与 `string` 不同的类型，从而阻止联合类型的简化：

     ```typescript
     type IStr = "A" | "B" | (string & {});
     // 联合类型不会被简化，保留 "A" | "B" | string
     ```
...
- **类型优先级**：TypeScript 在推断字面量类型时，会优先保留更具体的类型（如 `"A"`）而非宽泛的 `string`。
- **交集类型的特殊性**：`string & {}` 会被视为一个 **非原始字符串类型**，但它仍然接受所有字符串值。编译器会将 `"A"` 直接匹配到字面量类型 `"A"` 而非 `string & {}`，因此保留提示。
...
以下代码会明确显示 `IStr` 的类型结构：

```typescript
type IStr = "A" | "B" | (string & {});
// 鼠标悬停查看类型，显示为 "A" | "B" | (string & {})
// 而非简化的 `string`
```
...
- `{}` 表示一个空对象类型（不包含 `null`/`undefined`）。
- `string & {}` 的交集类型实际上是所有 `string` 类型的值（因为字符串属于原始类型，但 TypeScript 中原始类型也符合 `{}` 的定义）。
- 逻辑上 `string & {}` 等价于 `string`，但 TypeScript 会将其视为一个“标记化”的类型，阻止联合类型简化。

2. 类型系统行为

TypeScript 的联合类型简化规则是：

- 如果联合类型中的某个成员是其他成员的父类型，则父类型会覆盖子类型。
- 但 `string & {}` 不会被识别为 `string` 的父类型，因此不会触发覆盖。
...
```typescript
type IStr = "A" | "B" | (string & { _brand?: never });
// 添加一个虚拟属性标记，同样阻止简化
```
...
通过 **交集类型的小技巧**，你可以保留字面量类型的提示，同时允许接受任意字符串值。这是 TypeScript 类型系统的一个灵活用法，但需谨慎使用以避免混淆。
