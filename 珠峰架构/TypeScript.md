...
在写原有的 JS 代码时加上代码变量的类型。ts 文件不能直接被 js 解析器直接解析后执行，需要将 ts 文件编译为 js 文件后，再由 js 引擎执行。
...
- 什么是 TS，为什么要使用 TS
- 怎么使用 TS：借助打包工具， TS 配置文件的生成（tsc --init）
...
**没有赋初始值的变量，默认类型就是一个 any, 在 ts 中尽可能避免使用 any，ts中不声明变量类型直接初始赋值特定类型的数据，ts可以自动的对变量进行类型检测和确定（类型推导）**
