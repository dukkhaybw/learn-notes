...
类型就是一系列值的集合体，对于集合来说，最小的集合是空集，它不包含任何值，在TS中与之对应的类型是never。第二小的集合是包含单个值的集合，如字面量类型。

typescript是一个JavaScript的超集，TS包含JS的所有新旧语法特性，其最主要的是引入了**变量类型**（类型校验），让JavaScript从**动态类型**的语言变为了**静态类型**的语言。     

![typescript](http://img.zhufengpeixun.cn/typescript.jpg)

**写代码时标注代码的类型**，在写原有的JS代码时加上代码变量的类型。ts文件不能直接被js解析器直接解析后执行，需要将ts文件编译为js文件后在有js引擎执行。
...
- **类型系统的语法提示**
- **利于开发大型企业应用**，避免开发过程中因为没有考虑全面变量可能的值的类型而报错
- **编译阶段类型系统的检查**



**类型推论**导致大部分类型都不需要手动声明。同时TypeScript 增强了编辑器（IDE）的功能，包括**代码补全、接口提示、跳转到定义、代码重构**等，这提高了开发效率。而且 TypeScript 有近百个[编译选项][]，如果认为类型检查过于严格，那么可以通过修改编译选项来**降低类型检查的标准**。 



什么是TS，为什么要使用TS，怎么使用TS：借助打包工具， TS配置文件的生成（tsc --init）

基本变量类型，数组，`<类型>variable`，元组，枚举，null 和 undefiend，never，类型别名 type，viod，object类型

非空断言 ！ as，字面量类型

函数类型，函数参数的默认值，可选参数和剩余运算符，指定函数的this类型，函数重载

TS中获取一个值的数据类型：typeof 

类，类的静态属性，静态方法，原型方法，原型属性，实例属性，属性访问器

类的属性修饰符：public，private，protected，readonly
...
每章的内容很详细，但是详细就意味着涉及其他章节的知识点，没有基础的话，比较难以消化和理解。所以可以在了解了基础知识的情况下再去学习参考官网内容，以求提高效率。
...
```shell
npm install rollup typescript  -plugin-typescript2 @rollup/plugin-node-resolve -D
```

rollup:打包js类库的工具

typescript：第三方模块，**内部提供许多的ts类型**，默认自带的

rollup-plugin-typescript2:关联rollup和typescript 这两个包的第三方库

@rollup/plugin-node-resolve:第三方模块的解析规则插件,在import某个文件时，自动找index.js文件。告诉rollup如何查找外部模块。
...
一般情况下写ts时，需要生成一个ts的配置文件以自定义一些规则。
...
- typescript
  - ts编译器
- ts-loader
  - ts加载器，用于在webpack中加载ts文件并连接typescript
- html-webpack-plugin
...
- @babel-loader
  - babel在webpack中的加载器
- core-js
  - core-js用来使旧版本的浏览器支持新版ES语法（polyfill）
...
tsconfig.json（使用typescript将源码转为指定版本）:
...
- 安装typescript解析器（将ts转为js），ts解析器使用nodejs写的      **npm install typescript -g**

使用：

- 创建一个ts格式的文件，写入ts代码
- 在文件所在目录中用命令行窗口打开，执行 tsc xxx.ts  命令将ts编译为js文件
...
## typescript语法
 ...
 ts （ts的第三方包和其他项目的第三方包都可以提供）中有很多类型 （TS库自带的内置类型：（先掌握基本类型， 自己去定义类型， ts中高级类型））； ts 最终会被编译成js （所添加的类型 最后都会被删除掉 ， 只是为了开发者来进行约束的，最终编译的结果就是一堆空气，在js中不可见）

 后续注意点：

1. 变量 ：后面的都是类型  
2. ts中要考虑安全性 如果是安全的就可以赋值 
3. **类型推导**，会根据赋值的值得类型来推导变量类型

如果没有赋初始值的变量默认类型就是一个any, 在ts中尽可能避免使用any
...
- 类型声明
  ...
  ```ts
  // ts中不声明变量类型直接初始赋值特定类型的数据，ts可以自动的对变量进行类型检测和确定（类型推导）
  let name ='JACK';
  name = 123;  // 提示错误
  ...
  // String / string 的区别  (大写开头的都叫类类型，小写开头叫基本类型)
  
  let num1: number = 12;
  let num2: number = Number(12); // 强制转化 最后也是一个number类型
  
  // 描述的是基本数字类型用number，描述数字     
  // 描述构造函数的实例用Number
  let num3: Number = new Number(12); // number 只能描述基本类型，所以不能来描述实例
  let num4: Number = 123; // 该行不报错，尽量不要用类类型来描述基本类型. 后续自定义类型会有大写的情况
  
  // 为什么vue props用的是类类型？  props里面放的是String,Number，它们是js中的函数不是ts中的类型概念
  
  // 包装的概念，隐式类型转换
  // 12..toFixed(); // 告诉js 没有小数位调用toFiexed方法
  console.log((12).toFixed(2));
  
  
  
  
  // 数组类型  js数组非常的宽泛 [1,'abc']  
  // 什么叫数组？存放一类类型的集合
  let arr: (string | number)[] = ['1', '2', 1, 2, 3]; // 从安全性考虑能否赋值
  
  // 泛型也可以声明数组（泛型）
  let arr1: Array<number | string> = ['1', '2', 1, 2, 3]; 
  ...
  // ts 中衍生了一种类型 元组  （数组不限制长度和内部存储的顺序，元组要限制长度和顺序）
  ...
  // 联合类型赋值， 联合类型的变量被赋值后类型就确定了
  let num5: undefined | number = undefined; // 如果有可能是undefined 可以采用联合类型。
  ...
  // 自定义类型
  type ICircle = { r: number; xx: 'circle' }; // xx标识叫可辨类型
  type ISquare = { width: number; xx: 'square' };  // 是ts语法，和enum差不多，都是声明类型类型别名
  ...
  function sum(): void {
    	// undefined 可以赋予给void 。  不能标识为undefiend  ， undefiend 只能赋予给undefiend
  ...
  // 字符串 数字 boolean 数组 元组 枚举 null undefiend  never void  object any
  // symbol bigInt
  
  let name = '123'; // 无法重新声明块范围变量“name”。 默认ts中有声明过name属性了，所以我们在是有的时候 不能在重新声明了
...
- 联合类型（Union Types）表示赋值可以为多种类型中的一种
...
### 类型断言（! 或者 as typeName  <xxx>value）

```ts
// 类型断言 不知道是什么类型 可以断定他是什么类型 （手动判断） 断言后出错了，就要自己承担
...
```ts
// ts可以给函数添加类型声明   函数的参数、函数的返回值，函数本身标识类型
...
function callThis(this:MyThis,value:string) { // 在ts中函数使用必须要标识this的类型
}
...
// 在开发类库 插件 都很适合使用ts， 使用者可以有代码提示。 写业务多数都会使用接口，限定数据存放的类型
...
将重载签名和实现签名结合后就是函数重载。 在调用函数时，传参只能时重载签名中的某一个。当ts编译器处理函数重载时它会查找重载签名列表，尝试使用第一个符合重载的定义。
...
类型断言（Type Assertion）可以手动指定一个变量的类型。写法：值 as 类型（推荐使用）  或者  <类型>值
...
function isFish(animal: Cat | Fish) {
    if (typeof (animal as Fish).swim === 'function') {   //  断言 as Fish
        return true;
    }
    return false;
}
...
- 将任何一个类型断言为 `any`
  有的时候，非常确定一段代码不会出错。将一个变量断言为any后，在 `any` 类型的变量上，访问任何属性都是允许的。（将一个变量断言为 `any` 可以说是解决 TypeScript 中类型问题的最后一个手段）
- 将 `any` 断言为一个具体的类型
  遇到 `any` 类型的变量时，我们可以选择无视它，任由它滋生更多的 `any`。也可以选择改进它，通过类型断言及时的把 `any` 断言为精确的类型，使代码向着高可维护性的目标发展。
...
```ts
// 在es6中类都要通过 class 来进行声明
// 类能new 创建实例，类在ts中可以当做类型也可以当做值. 类类型可以描述实例

type a = String   //类充当类型

//当开发者在使用this及其上面的属性的时候，得先描述this有哪些属性
...
class Circle {
    constructor( public x:number,public y:number, public r:number){   //直接将参数定义到this上了，不用再写this.x = x this.y = y  this.r = r
        ...
        // this.y = y
        // this.r = r
        // 没有写 public 得参数默认不会自动挂载到this上，可以手动挂载
    }
}
...
class Circle {
    ...
    _r:string
    constructor(public x:number,public y:number, r:{name:string}){ 
        // this.x = x;
        // this.y = y;
        this._r = r.name; // 将传进来的值r 放在this._r上
    }
}

let circle = new Circle(100,100,{name:'zf'});
console.log(circle)
...
class Animal{
    public name: string 
    protected constructor(name: string) {  // 抽象类，不能被new调用
        this.name = name;
    }
}
...
class Animal{
    public name: string 
    private constructor(name: string) {  // 不用被new调用，也不能被继承了
        this.name = name;
    }
}
...
// 和es6中类的使用是一致的 唯一区别是多了修饰和和类型声明，其他用法可以共享
...
![image-20220119113643545](C:\Users\dukkha\AppData\Roaming\Typora\typora-user-images\image-20220119113643545.png)
...
1)  type可以使用联合类型  interface 不支持联合类型
...
2) interface同名可以合并， type重名会报错
   ...
   interface IFruit {  // 在上面的 IFruit的接口基础上声明一个同名的接口，只是接口中规定的内容不同
       age:number
   }
...
![image-20220118093253634](C:\Users\dukkha\AppData\Roaming\Typora\typora-user-images\image-20220118093253634.png)
...
- 类不能继承接口，只能实现，并且类可以实现多个接口；但是类可以继承抽象类，类只能一次继承一个类
  ...
  class Cat extends Animal {  // 继承抽象类
      name = '猫'
      eat(): void {
          throw new Error("Method not implemented.")
      }
  ...
  class Tom implements IEat, IDrink {    // 实现，并实现多个就扣
      public eat: () => void 
      public constructor() {
          this.eat = () => { }
      }
...
interface V2 {x:number; y:string}
...
了解TS中的类型和TS所采用的结构化类型系统是TS进阶的关键。
...
function swap<T, K>(tuple: [T, K]): [K, T] { // 这里可以使用多个泛型， 泛型会依靠位置来进行猜测
...
let name!:never
let person3: IPerson3 = { // person3是交叉后的结果
    handsome: '帅',
    high: '高',
    name, // 因为无法相交
}
...
type DeepPartial<T> = { // 原生的是不支持深度递归的
    // 可以通过条件判断递归ts的类型
...
type Readonly<T> = {   //  Readonly表示只读   原生实现支持
...
//     [X in K]: V // for(x in K)
// }
function mapping<K extends keyof any, V, R>(obj: Record<K, V>, cb: (key: K, value: V) => R): Record<K, R> {
...
// 这里不关心对象的组成   映射成另一种结构
let mapResult = mapping({ a: 1, b: 2, c: 'abc' }, (key, value) => { // ={a:3, b:6 , c:9}
    return value
})
...
function isCircle(val: ICircle | IRant): val is ICircle { //  val is ICircle 意味着返回值为true就是圆， 自定义类型保护
    return val.kind == 'circle'
}
function getArea(val: ICircle | IRant) { // true是圆  还是false是圆
    if (isCircle(val)) {
        val
    }
}
...
function isObject(val:any):val is object{    // 自定义类型保护
...
理解为TS内置的隐含的规范，确定多个变量之间能否彼此相互赋值。不是具体的应用场景而是编写规范。（什么样的变量何可以给什么样的变量赋值）
...
// 传父亲 （参数是逆变的，可以传递父亲）
// 返儿子 （返回值是协变的 可以返回孩子）
function fn(cb: (val: Son) => Son) { // val 是son 说明可以处理 money和car
...
// 类可以描述实例充当实例的类型
// 类的类型需要typeof取值
type InstanceType<T extends { new(...args: any[]): any }> = T extends { new(...args: any[]): infer Instance } ? Instance : never
...
// 直接就合并到了全局的接口上
interface String { //interface 同名的可以合并
    xx:()=>void
}
...
declare module '*.vue'{ // shim-vue.d.ts
...
### 自定义类型
  ...
  在没有做任何ts配置时，默认情况即使因为变量的类型提示有错误存在，ts任然是可以被编译为js的。之后为了避免这种情况，可以书写一个ts的配置文件。

注意：ts文件是可以有选择的被编译为任意特定版本的js（es3， es5，es6....）文件的,有针对的提高兼容性。之后可以通过对ts工具进行配置，以编译为指定的js版本。 





对于js中的函数而言，它是不会考虑函数形参的类型和个数的。
...
一个声明文件就是一个全局的文件，可以在任何地方使用。后缀是.d.ts声明的。ts会检测根目录下及其后代目录下的所有.d.ts文件。我们一般不建议将.d.ts辅助文件放到src目录下。
...
一般项目中会有一个和src平级的目录，typing放自己的 或者 types放第三方.d.ts 文件
...
在项目中，当使用一些第三方包的时候，这些没有对ts进行支持的第三方包暴露的变量或者函数，直接在项目中使用时，因为ts没有检测到对应的数据类型而会提示错误，这时就需要自己去定义全局的.d.ts文件去解决报错提示。
...
![image-20210711154756553](C:\Users\dukkha\AppData\Roaming\Typora\typora-user-images\image-20210711154756553.png)
...
- 直接使用字面量进行类型声明
  ...
  在js中，有一种函数连undefined都不会返回，主要是用于报错的
