...
类型就是一系列值的集合体，对于集合来说，最小的集合是空集，它不包含任何值，在 TS 中与之对应的类型是 never。第二小的集合是包含单个值的集合，如字面量类型。

typescript 是一个 JavaScript 的超集，TS 包含 JS 的所有新旧语法特性，其最主要的是引入了**变量类型**（类型校验），让 JavaScript 从**动态类型**的语言变为了**静态类型**的语言。

![typescript](http://img.zhufengpeixun.cn/typescript.jpg)

**写代码时标注代码的类型**，在写原有的 JS 代码时加上代码变量的类型。ts 文件不能直接被 js 解析器直接解析后执行，需要将 ts 文件编译为 js 文件后在有 js 引擎执行。
...
**类型推论**导致大部分类型都不需要手动声明。同时 TypeScript 增强了编辑器（IDE）的功能，包括**代码补全、接口提示、跳转到定义、代码重构**等，这提高了开发效率。而且 TypeScript 有近百个[编译选项][]，如果认为类型检查过于严格，那么可以通过修改编译选项来**降低类型检查的标准**。

什么是 TS，为什么要使用 TS，怎么使用 TS：借助打包工具， TS 配置文件的生成（tsc --init）

基本变量类型，数组，`<类型>variable`，元组，枚举，null 和 undefiend，never，类型别名 type，viod，object 类型
...
- typescript
  - ts 编译器
- ts-loader
  - ts 加载器，用于在 webpack 中加载 ts 文件并连接 typescript
- html-webpack-plugin
...
ts （ts 的第三方包和其他项目的第三方包都可以提供）中有很多类型 （TS 库自带的内置类型：（先掌握基本类型， 自己去定义类型， ts 中高级类型））； ts 最终会被编译成 js （所添加的类型 最后都会被删除掉 ， 只是为了开发者来进行约束的，最终编译的结果就是一堆空气，在 js 中不可见）
...
如果没有赋初始值的变量默认类型就是一个 any, 在 ts 中尽可能避免使用 any
