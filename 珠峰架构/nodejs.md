...
Buffer 是 global 的属性，属性的值是一个构造函数。服务器端需要大量操作文件，所以 node 就自定义了一个类型 buffer，代表的是内存中存放的二进制数据，可以直接访问和操作二进制数据。操作文件就是 i/o 操作，针对内存做输入输出，描述内存情况，多大的文件，文件的内容。buffer在打印使用的是内存地址上存放的数据，但是buffer中实际存放的是该数据对应的内存地址。
...
读写分离。读取可以分段读取，可暂停，可恢复，这里是基于文件模块的可读流。（流的概念不一定都是用在文件读写上的，网络IO也有读写流）
...
1. 编写的时候，业务逻辑不清晰，同一个方法中有大量的如下判断，不好维护和拆分
...
koav1中使用的是生成器函数。koav2采用的是async/await。

![image-20240504182116363](images\image-20240504182116363.png)
...
express处理异步的方式都是回调，koa主要使用的是async和await。express框架体积比kkoa大一些，同时本身内置了一些中间件，不用像koa那样需要基本的中间件都需要自己去写或者配置。
...
let routes = [{
    ...
    method:'*',
    handle(req,res){
        res.end(`Cannot ${req.method.toLowerCase()} ${req.url}`)
    }
}]
            ...
            const server = http.createServer((req,res)=>{
                ...
                for(let i= 1;i<routes.length;i++){
                    ...
                    if(path === requestUrl && method === requestMethod){
                        return handler(req, res)  // return 语句说明只要匹配到一项后，后面即使再有相同的api也不会被触发
                    }
                }
...
- Cookie 是一种存储在用户浏览器中的小型文本数据，用于在客户端和服务器之间交换信息

- 每次请求浏览器都会自动携带 cookie 随请求头发送给服务器

- 同一个域名下设置**多个 cookie**，并且每个 cookie 的大小限制通常是 **4KB 左右**（包括键和值的总大小），

- 设置 `key=value`，整个键名 `key` 和键值 `value` 的长度加起来不能超过约 4KB。包括一些额外的属性（如 `HttpOnly`、`Secure`、`Expires`）也会占用字节数。

  浏览器通常允许每个域名设置 **20~50 个 cookie**，具体限制因浏览器而异。

  - 如果超出这个限制，旧的 cookie 可能会被删除。

  某些现代浏览器对单个域名的总 cookie 存储大小也有更高的限制（例如，**10KB~4096KB**），但这个限制涵盖了所有 cookie 的总大小。

- 如果未设置 `expires` 或 `max-age`，cookie 在浏览器会话结束后失效（会话 cookie）

- 如果设置了有效期，则为持久性 cookie

- 作用域：受域名和路径限制，仅能被指定的域名访问

- 可设置 `HttpOnly` 防止 JavaScript 访问

- 可设置 `Secure` 强制仅通过 HTTPS 发送

- 一级域名中设置的cookie能在二级域名中也被共享

cookie 内容过多可能导致页面白屏，会浪费流量。尽量 cookie 合理化，如通过过期时间，通过域名分割 cookie。默认可以限制二级域名共享 cookie。cookie 会走网络，所以有安全问题。
...
> **Cookie 的路径作用域**是指一个 Cookie 可以被哪些 URL 路径访问和使用。这是由 Cookie 的 `Path` 属性决定的。通过设置 `Path`，开发者可以控制 Cookie 的作用范围，确保它只在特定的路径下被发送到服务器。
>
> **Cookie 的路径作用域**是指一个 Cookie 可以被哪些 URL 路径访问和使用。这是由 Cookie 的 `Path` 属性决定的。通过设置 `Path`，开发者可以控制 Cookie 的作用范围，确保它只在特定的路径下被发送到服务器。
>
> ------
...
> 1. **默认行为**：
>
>    - 如果没有显式指定 Path 属性，Cookie 的路径默认为创建它的 URL 的路径（不包含文件名部分），例如：
>      - 如果在 `/example/index.html` 设置了一个 Cookie 且未指定 `Path`，它的路径默认为 `/example/`。
>    - 该 Cookie 会对  /example/ 路径及其子路径生效，例如：
>      - `/example/`, `/example/subpage/`, `/example/subpage/file.html`
>
> 2. **显式指定 `Path`**：
>
>    - 开发者可以通过设置 Path属性来自定义路径作用域，例如：
>
>      ```http
>      Set-Cookie: user=John; Path=/account;
>      ```
>
>    - 这个 Cookie 仅对 `/account/` 及其子路径（如 `/account/settings/`）生效，但对 `/example/` 无效。
>
> 3. **子路径继承**：
>
>    - 指定路径的 Cookie 会自动对其所有子路径生效，例如：
>      - 设置 `Path=/example/`，则 `/example/`, `/example/subpage/`, `/example/subpage/file.html` 都能访问该 Cookie。
>
> 4. 在同一个域名下，不同路径可能会设置相同名称的 Cookie。通过路径隔离可以避免冲突。
>
>    ```http
...
>    
>    
>    Set-Cookie: user=John; Domain=sub.example.com; Path=/admin;   路径作用域可与 Domain 属性配合，进一步细化控制。此 Cookie 仅适用于 sub.example.com/admin 及其子路径。
>    ```
>
>    `/shop` 下的页面获取的 `theme` 是 `dark`。
>
>    `/profile` 下的页面获取的 `theme` 是 `light`。
...
- Session 是一种服务器端的会话存储机制，用于保存用户会话信息（例如登录状态）
- 通常结合 cookie 使用，服务器通过 cookie 存储的会话 ID 识别用户并获取对应的会话数据
- 数据存储在服务器端，客户端仅保留一个 session ID
- 没有明显限制，取决于服务器存储能力
- 在用户关闭浏览器或会话超时（由服务器设置的时间限制）后失效
- 数据存储在服务器，更安全，但需防范 session 劫持攻击
...
```js
//koa中操作cookie
ctx.req.headers.cookie; // 取

ctx.res.setHeader('Set-Cookie', 'name=zf; domain=xx.xxx.com; path=/; max-age=n; httpOnly=true'); // 设置一个   n秒

ctx.res.setHeader('Set-Cookie', ['name=zf', 'b=1']); // 设置多个
...
`sessionStorage` 是 HTML5 提供的浏览器端存储机制，用于在会话期间存储数据。

- **存储位置**：仅存在于当前的浏览器标签页。
- **大小限制**：一般限制为 5MB。
- **生命周期**：会话级别，关闭浏览器标签页时清空。
- **作用域**：仅限当前页面的标签页，不能跨标签页或窗口共享。



`localStorage` 也是 HTML5 提供的浏览器端存储机制，但与 `sessionStorage` 不同，它用于持久化存储数据。

- **存储位置**：持久化存储在浏览器中。
- **大小限制**：一般限制为 5MB。
- **生命周期**：持久存储，除非手动清除或程序清除，否则不会自动过期。
- **作用域**：同一浏览器下的相同域名页面可共享数据。



| 特性              | **localStorage**                                           | **sessionStorage**                                   |
| ----------------- | :--------------------------------------------------------- | ---------------------------------------------------- |
| **生命周期**      | 持久化存储，除非手动删除，否则数据不会失效。               | 仅在当前会话（浏览器标签页）有效，关闭标签页即清除。 |
| **作用范围**      | 同一浏览器下，同源（相同域名、协议、端口）的页面共享数据。 | 同一标签页或窗口内的同源页面共享，不跨标签页。       |
| **数据大小限制**  | 通常为 5MB                                                 | 通常为 5MB                                           |
| **跨窗口/标签页** | 可以跨标签页、窗口共享数据（同源）。                       | 不能跨标签页或窗口，每个标签页有独立的存储。         |
...
自己实现的http-server存在的问题：

- 编写的时候业务逻辑不清晰，导致业务不好拆分和维护
- 很多公共方法没法复用
- 错误处理没有统一
- 难和别人的库进行集成

koa就对这些痛点进行解决，它用es6语法封装了web服务。库的核心代码量少，能通过插件实现方便的扩展自己的功能。
...
![image-20241219222012336](D:\learn-notes\珠峰架构\images\image-20241219222012336.png)

### 常见文件及功能

1. **`application.js`**
   - **功能**：Koa 的核心文件，定义了 `Application` 类。负责框架的整体架构设计。
   - 主要职责：
     - 创建和管理 HTTP 服务器。
     - 处理中间件的加载和执行。
     - 提供基础方法，如 `listen`（启动服务）、`use`（加载中间件）等。
     - 在请求和响应之间建立上下文（`ctx`）。
     - 调用中间件的核心机制（`compose` 中间件链）。
2. **`context.js`**
   - **功能**：定义了 `ctx` 对象（上下文）的原型，`ctx` 是请求和响应的桥梁。
   - 主要职责：
     - 提供便捷的属性和方法访问 HTTP 请求和响应数据。
     - 为用户和中间件提供统一的接口，例如 `ctx.request`、`ctx.response`、`ctx.body` 等。
3. **`request.js`**
   - **功能**：封装 HTTP 请求相关的内容。
   - 主要职责：
     - 提供对请求信息的访问接口，例如 `url`、`method`、`headers` 等。
     - 增加便捷属性，例如 `query`（解析的查询字符串对象）、`path`（请求路径）等。
4. **`response.js`**
   - **功能**：封装 HTTP 响应相关的内容。
   - 主要职责：
     - 提供对响应的统一接口，例如 `status`、`headers`、`body` 等。
     - 处理响应头、响应状态码的设置和获取。
5. **`middleware.js`**
   - **功能**：实现中间件的组合逻辑（`compose`）。
   - 主要职责：
     - 负责按顺序调用中间件，并正确处理异步逻辑。
     - 确保中间件链的洋葱模型（前后流程）的执行顺序。
6. **`util.js`**（可选，某些版本可能存在）
   - **功能**：包含框架内部使用的辅助工具函数。
   - 主要职责：
     - 提供通用功能，如类型检查、错误处理、流转换等。

### 文件之间的关系

- **`application.js` 是入口**：`application.js` 是核心模块，负责创建 Koa 实例并管理整个应用程序。其他文件如 `context.js`、`request.js` 和 `response.js` 则提供具体的请求和响应封装。
- **`context.js` 聚合请求和响应**：通过组合 `request.js` 和 `response.js`，将它们挂载到 `ctx` 上，为中间件和用户代码提供统一的接口。
- **`middleware.js` 执行中间件链**：负责管理中间件的执行顺序，是 Koa 洋葱模型的关键实现。
...
以插件的形式实现一个个的功能。中间件就是一个函数，函数会接受koa传递过来的参数ctx，next。



### 实现1

lib/application.js，入口文件，package.json中main字段指向的文件。
...
mongoDB的用法，在node中如何使用mongoDB。

- 关系型数据库 rdb， 以表存储数据，分为行和列，一行为一条数据，每一列的数据必须相似，存储数据的格式是固定的，不够灵活
...
show collections
db.collectionName.insert(dataValue)
db.collectionname.drop()


use admin
db.createUser(jsonValue)
```
...
**Node.js 是一个基于 Chrome V8 JavaScript 引擎的跨平台、事件驱动、非阻塞 I/O 的 JavaScript 运行时环境**。它用于在服务端或非浏览器环境中运行 JavaScript 代码。它打破了 JavaScript 只能在浏览器中运行的限制，使得开发者可以使用 JavaScript 构建服务端应用。



**Node.js 的核心特点**

- **非阻塞 I/O（Non-blocking I/O）和事件驱动架构**：
  - Node.js 采用异步事件驱动模型，非常适合高并发场景。
  - 单线程模型通过事件循环机制处理大量连接，而无需为每个请求创建线程。
- **基于 Chrome V8 引擎**：
  - V8 引擎将 JavaScript 编译为高效的机器码，提供卓越的性能。
- **模块化支持**：
  - 遵循 **CommonJS** 规范，同时支持 ES Modules（从 Node.js 14 开始稳定支持）。
  - Node.js 的模块加载机制允许开发者引入内置模块或第三方模块（通过 npm）。
- **核心架构**：
  - **事件驱动**： Node.js 的事件循环（Event Loop）基于 `libuv` 实现，用单线程模型处理高并发请求。所有 I/O 操作都通过异步事件完成。
  - **非阻塞 I/O**： 使用回调、Promise 或 async/await 等机制，让线程避免被 I/O 操作阻塞。



**Node.js 的应用场景**

- **Web 服务**：使用框架如 Express.js 或 Koa.js 构建高性能的 Web 服务。
- **实时应用**：如聊天系统、在线游戏、实时协作工具（通过 WebSocket）。
- **API 服务**：为前端应用或移动端提供 RESTful 或 GraphQL API。
- **任务自动化**：如脚本任务、构建工具（Webpack、Gulp 等）。
- **微服务架构**：构建分布式微服务系统。
...
1. **高性能**：异步 I/O 和 V8 引擎让其适合高并发场景。
2. **全栈开发**：使用 JavaScript 构建前后端，提高开发效率。
3. **活跃的生态系统**：拥有丰富的第三方模块库（通过 npm）。
4. **跨平台支持**：一次开发，运行在多种平台。

**缺点：**

1. **单线程模型**：对于 CPU 密集型任务性能较差，需引入多线程（`worker_threads`）或其他解决方案。
2. **调试难度**：异步代码可能导致调试复杂性增加。
3. 不适合CPU密集的应用
4. 因为是单线程机制，代码发生问题会导致系统崩溃
...
`fs` 模块是 Node.js 中的 **文件系统模块**，用于与文件和目录进行交互操作。通过 `fs` 模块，开发者可以进行文件的创建、读取、写入、删除等操作，还可以操作目录结构。

文件权限位（mode）是一种用于控制文件或目录访问权限的**标志**。它们源于 UNIX 文件系统权限模型，通过八进制数字表示**文件或目录**的权限，主要分为以下三类用户组：

1. **所有者（Owner）** - 文件的拥有者。
2. **所属组（Group）** - 文件的所属组。
3. **其他人（Others）** - 除所有者和所属组之外的其他用户。

每个用户组有三个基本权限：

1. **读取权限（Read, `r`）** - 是否可以读取文件内容或列出目录内容。
2. **写入权限（Write, `w`）** - 是否可以修改文件内容或在目录中创建/删除文件。
3. **执行权限（Execute, `x`）** - 是否可以执行文件（如脚本）或进入目录。
...
- `777` 表示所有用户组都有读取、写入和执行权限。
- `644` 表示所有者有读取和写入权限，其他用户组只有读取权限。
- `755` 表示所有者有全部权限，其他用户组有读取和执行权限。



**在 Node.js 中操作权限位**

Node.js 提供了一些方法来操作文件权限，例如：

1. **`fs.chmod` 和 `fs.chmodSync`**
...
fs.chmod('example.txt', 0o644, (err) => {
  if (err) throw err;
  console.log('Permissions changed to 644');
});
...
`stats.mode` 包含文件权限位，可以通过位操作提取特定的权限信息。
...
- **粘滞位（Sticky Bit）**：`1xxx`，通常用于目录，只有文件的所有者或根用户可以删除其中的文件。
- **SetUID（Set User ID）**：`4xxx`，执行文件时，进程以文件所有者的权限运行。
- **SetGID（Set Group ID）**：`2xxx`，执行文件时，进程以文件所属组的权限运行。
...
通过这些方法，可以灵活地操作和检查文件的权限位。



### 文件操作模式

`fs` 模块中，文件操作（如打开文件时）需要指定文件的 **模式（flag）**，这些模式用字符串表示，控制文件的打开方式和权限行为。
...
| **模式**  | **含义**                                                     |
| --------- | ------------------------------------------------------------ |
| **`r`**   | 以读取模式打开文件。如果文件不存在，会抛出异常。             |
| **`r+`**  | 以读写模式打开文件。如果文件不存在，会抛出异常。             |
| **`w`**   | 以写入模式打开文件。如果文件已存在，会清空其内容；如果文件不存在，则会创建文件。 |
| **`w+`**  | 以读写模式打开文件。如果文件已存在，会清空其内容；如果文件不存在，则会创建文件。 |
| **`a`**   | 以追加模式打开文件。如果文件不存在，则会创建文件。           |
| **`a+`**  | 以读取和追加模式打开文件。如果文件不存在，则会创建文件。     |
| **`ax`**  | 以独占的追加模式打开文件。如果文件已存在，则操作会失败。     |
| **`ax+`** | 以独占的读取和追加模式打开文件。如果文件已存在，则操作会失败。 |
| **`wx`**  | 以独占的写入模式打开文件。如果文件已存在，则操作会失败。     |
| **`wx+`** | 以独占的读写模式打开文件。如果文件已存在，则操作会失败。     |
...
4. **读写模式（清空内容）(`w+`)**：
   ...
   fs.open('example.txt', 'w+', (err, fd) => {
       if (err) throw err;
       console.log('File opened for reading and writing (content cleared).');
   });
   ```

5. **追加模式 (`a`)**：

   - 只能写入数据，并且新写入的数据会追加到文件末尾。
   - 如果文件不存在，则会创建文件。

   ```js
   fs.open('example.txt', 'a', (err, fd) => {
       if (err) throw err;
       console.log('File opened for appending.');
   });
   ```

6. **读取和追加模式 (`a+`)**：

   - 可以读取文件内容，也可以将新内容追加到文件末尾。
   - 如果文件不存在，则会创建文件。

   ```js
   fs.open('example.txt', 'a+', (err, fd) => {
       if (err) throw err;
       console.log('File opened for reading and appending.');
   });
   ```
...
7. **独占模式 (`x`) 和 (`wx`)**：
   ...
   fs.open('example.txt', 'wx', (err, fd) => {
       if (err) {
           console.error('File already exists or error occurred.');
       } else {
           console.log('File created exclusively for writing.');
       }
   });
   ```
