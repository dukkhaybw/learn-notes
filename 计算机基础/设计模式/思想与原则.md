...
- 用面向对象编程语言写出来的代码，不一定是面向对象编程风格的，有可能是面向过程编程风格的。相反，用面向过程编程语言照样也可以写出面向对象编程风格的代码。尽管面向过程编程语言可能没有现成的语法来支持面向对象的四大特性，但可以通过其他方式来模拟，比如在 C 语言中，可以利用函数指针来模拟多态。一门面向过程的编程语言，如何用它来模拟面向对象的四大特性？
...
在 TypeScript 中，可以**使用抽象类来模拟接口**的一部分功能。抽象类可以定义方法的签名，但不能提供实际的实现。其他类可以继承这个抽象类并提供具体的实现。
...
在 TypeScript 中，可以**使用普通类来模拟接口**。与抽象类不同，普通类可以提供方法的具体实现。然而，使用普通类作为接口时，**通常会将方法留空或者提供一个默认实现**，以便其他类可以根据需要选择性地覆盖这些方法。
