...
> 享元模式（**结构型**），用于**减少创建相似对象的数量**，减少内存占用。其核心思想是**共享细粒度对象**（对于需要反复创建的对象，每次创建前先去缓存中查找，找到就返回重复利用，没有找到就创建然后加入到缓存供给下次使用）。
>
...
数据结构与算法解决代码效率问题。 设计模式解决代码质量问题（**可扩展，可读，可维护等**），做好代码review。
