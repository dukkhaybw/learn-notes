使用html5标签和css实现静态页面的布局。

- PC端页面布局
- 移动端页面布局
- 响应式布局

目的：熟练掌握网页布局，能高度实现看到的页面的静态效果。

- html5部分学习常用的html标签；
- css3部分学习装饰页面的css样式属性和规则；
- html5和css3新增的标签，属性和规则；
- 模拟实现品优购页面项目
...
HTML：超文本标记语言（Hyper Text Markup Language），它是用来描述页面的骨架的标签语言，所以它有一套规定的标签构成。
...
1. 结构
   ...
   用于确定网页标签（元素）的类别和使用规范，即html，页面中右键查看网页源代码的结果就是页面的结构
...
- 标签和标签之间一般存在两种可能的关系（注意一般行类元素不会包含块级元素）：
...
- `<html> `元素是 HTML 页面的根元素，常见的标签属性是lang='en | zh-CN | 等'，能给浏览器一个提示说明本html文件是什么语言的页面，只是一个提示作用，具体页面是什么语言并不由这个字段确定
- `<head> `元素包含了文档的元（meta）数据，如 `<meta charset="utf-8">` 定义网页编码格式为 **utf-8**。对于中文网页一般使用 **<meta charset="utf-8">** 声明编码，否则会出现乱码。
- `<title>` 元素描述了文档的标题
- `<body> `元素包含了可见的页面内容
...
每个标签都存在一定的语义，标签的作用。每种标签都会默认有一些列自己的css样式。根据语义，在合适的地方使用合适的标签（语义化），清晰页面的结构。
...
4. 文本格式化标签
   ...
   ​	<img src="C:\Users\dukkha\Desktop\learn-notes\css\images\image-20240123213312585.png" alt="image-20240123213312585" style="zoom:150%;" />
...
6. span标签
   ...
   3. 段落元素：`<p>`, `<blockquote>`, `<pre>`, 等用于表示段落或引用的元素不应该放在`<span>`内部。
   4. 标题元素：`<h1>`-`<h6>`用于表示标题的元素不应该放在`<span>`内部。
   5. 列表元素：`<ul>`, `<ol>`, `<li>`, 等用于创建列表的元素不应该放在`<span>`内部。
...
7. 图像标签
   ...
   其中，`src`属性**指定图像文件的路径或URL**，`alt`属性用于提供图像的替代文本，以便**在图像无法显示或无法访问时提供文字描述**。
   ...
   需要注意的是，`<img>`标签只是用于插入图像，它并不具备图像处理的能力。如果需要对图像进行调整、裁剪或添加特效等操作，可以使用CSS样式或JavaScript来实现。
