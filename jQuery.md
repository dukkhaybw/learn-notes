...
jQuery的版本一是最常用的,1.11.3版；jQuery的版本二去掉了很多pc端兼容代码，主要面对移动端，现在移动端用zpeto库代替了jquery；jQuery的版本三只是在版本一基础上进行的改善。



jQuery这个类库到底是什么？库中封装的是许多方法，通过调取方法开发项目。

它是能批量操作DOM的函数库，通过$('css选择器')能获取一组dom元素，如果直接对返回结果执行某个方法，则能对返回对象中的每个具体的dom元素都进行操作。它的链式操作非常简洁容易维护，解决了跨浏览器的兼容问题，并对Ajax进行了封装。
